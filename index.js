...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' MAKASIH UDAH ORDER DI ISALL GANS'))
})
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner BOT-ADIT`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena BOT-ADIT baik Bot memberikan ${one}Xp >_<`)
                      }else{
...
*/
case 'bc':
							if (!isOwner) return reply('*owner bukan luu?*') 
...
*/					
                case 'indohot':
				 client.updatePresence(from, Presence.composing)
					if (!isOwner) return reply('FITUR KHUSUS BOT-ADIT')				
